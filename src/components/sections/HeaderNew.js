...
const HeaderNew = ({ blok }) => {
    ...
    return (
        <motion.header
            className={`fixed z-50 w-full bg-white py-4 transition-all duration-500 ease-in-out lg:h-40 ${showHeader ? 'translate-y-0 opacity-100 shadow-sm' : 'pointer-events-none -translate-y-full opacity-0 shadow-none'}`}
            initial="closed"
            animate={isOpen ? 'open' : 'closed'}
        >
            <ContentWidth>
